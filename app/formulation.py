...
from seroepi.constants import AggregationType, PlotType, FormulationStrategy
...
def formulation_ui():
    ...
    return ui.layout_sidebar(
        ui.sidebar(
            ui.accordion(
                ui.accordion_panel(
                    "Formulation Design",
                    ui.tooltip(
                        ui.input_slider("max_valency", "Trait Valency", min=2, max=30, value=10, step=1),
                        "The maximum number of distinct variants to include in the optimal formulation formulation.",
                    ),
                    ui.tooltip(
                        ui.input_selectize("form_holdout", "Cross-Validation Stratum", choices=["Awaiting Data..."]),
                    ...
                    ui.tooltip(
                        ui.input_select(
                            "form_designer",
                            "Designer Type",
                            choices=FormulationStrategy.designer_ui_labels(),
                        ),
                ...
                ui.accordion_panel(
                    ...
                    icon=icon_lucide("wrench"),
                ),
                ui.accordion_panel(
                    "Import / Export",
                    ui.p(
                        "Save rigorous CV formulations to avoid recalculating, or load an existing one.",
                        class_="text-muted small",
                    ),
                    ui.input_file("formulation_upload", "Load Formulation (.pkl)", accept=[".pkl"]),
                    ui.output_ui("formulation_export_ui"),
                    icon=icon_lucide("save"),
                ),
                id="formulation_accordion",
                open=["Formulation Design"],
                multiple=True,
            ),
            width=350,
        ),
        ui.navset_card_tab(
            ui.nav_panel(
                "Rankings", ui.output_ui("form_rankings_content"), value="tab_form_rankings", icon=icon_lucide("trophy")
            ),
            ui.nav_panel(
                "Stability Metrics",
                ui.output_ui("form_stability_content"),
                value="tab_form_stability",
                icon=icon_lucide("scale"),
            ),
            ui.nav_panel(
                "Permutations",
                ui.output_ui("form_history_content"),
                value="tab_form_permutations",
                icon=icon_lucide("refresh-cw"),
            ),
            ui.nav_panel(
                "Plots", ui.output_ui("form_plots_content"), value="tab_form_plots", icon=icon_lucide("chart-bar")
            ),
            id="formulation_tabs",
        ),
    )
...
def formulation_server(input, output, session, app_state: dict):
    shared_df = app_state["shared_df"]  # noqa: F841
    ...
    results_registry = app_state["results_registry"]  # noqa: F841
    ...
    def update_form_inputs():
        ...
        if holdouts:
            ...
            ui.update_selectize(
                "form_holdout", choices=["Global (CV Not Possible)"], selected="Global (CV Not Possible)"
            )
    ...
    def update_custom_traits_choices():
        res = prev_results.get()
        if res is not None:
            all_traits = res.data["target"].unique().to_list()
    ...
    async def generate_optimal():
        ...
        if not holdout or "Awaiting Data" in holdout:
            ui.notification_show(
                "Please wait for valid burden estimates before designing a formulation.", type="warning"
            )
            return
        ...
        async with ui_task("Designer Error") as p:
            ...
            def ui_progress_callback(current, total):
                ...
                if is_global:
                    ...
                    message = (
                        f"Running CV Fold {current}/{total}..."
                        if designer_type != "posthoc"
                        else f"Running Permutation {current}/{total}..."
                    )
            ...
            if designer_type == "posthoc":
                ...
                await to_thread(designer.fit, est, agg_df, loo_col=loo_col, progress_callback=ui_progress_callback)
    ...
    def copy_to_custom():
        if formulation := current_formulation.get():  # type: Formulation  # noqa: PYI033
    ...
    def formulation_export_ui():
        if current_formulation.get() is not None:
            return ui.download_button(
                "btn_download_formulation", "Download Formulation", class_="btn-outline-primary w-100 mt-2"
            )
        return ui.div()

    @render.download_button(filename=lambda: "vaccine_formulation.pkl")
    ...
    def form_plots_content():
        if current_formulation.get() is None:
            return ui.div(
                "Configure and generate an optimal formulation in the sidebar.",
                class_="text-center mt-5 text-muted fs-4",
            )

        return ui.layout_sidebar(
            ui.sidebar(export_settings_ui("form"), ui.output_ui("dl_stability_btn_ui"), width=280),
    ...
    def form_stability_content():
        vac = current_formulation.get()
        if vac is None or vac.stability_metrics.is_empty():
            return ui.div(
                "Run Cross-Validated designer to view stability metrics.", class_="text-center mt-5 text-muted fs-4"
            )
    ...
    def form_history_content():
        ...
        if vac is None or vac.stability_metrics.is_empty():
            return ui.div(
                "Run Cross-Validated designer to view permutation history.", class_="text-center mt-5 text-muted fs-4"
            )
    ...
    def stability_plot_card():
        vac = current_formulation.get()
        if vac is not None and not vac.stability_metrics.is_empty():
            return ui.card(ui.card_header("Cross-Validation Stability Matrix"), safe_plot_ui("stability_plot"))
        return ui.div()
    ...
    def dl_stability_btn_ui():
        ...
        if vac is not None and not vac.stability_metrics.is_empty():
            return ui.download_button(
                "btn_download_stability", "Download Stability Plot", class_="btn-outline-primary w-100"
            )
        return ui.div()

    @render.download_button(filename=lambda: f"formulation_stability.{input.form_plot_format()}")
    def btn_download_stability():
        ...
        def save_fig(p: Path):
            fig.write_image(
                p, format=input.form_plot_format(), width=input.form_plot_width(), height=input.form_plot_height()
            )
    ...
    dt_download_server(
        "form_stability",
        data_callable=lambda: current_formulation.get().stability_metrics if current_formulation.get() else None,
