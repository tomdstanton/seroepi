...
from seroepi.constants import DistanceFlavour, Domain, GenotypeFlavour, PlotType, SpatialResolution, TemporalResolution
from seroepi.dist import GenomicDistances
from seroepi.domains import render_plot
from seroepi.io import BaseGenotypeParser, PathogenwatchGenomeParser

from .icons import icon_lucide
from .utils import ColMapper, build_grouped_choices, dt_download_server, dt_download_ui, ui_task, update_registry
...
def dataset_ui():
    ...
    return ui.layout_sidebar(
                ...
                ui.accordion_panel(
                    ...
                    ui.navset_pill(
                        ui.nav_panel(
                            ...
                            ui.div(
                                ui.tooltip(
                                    ui.input_file("genotype_file", "Genotype file (Required)", accept=[".csv"]),
                                    "Upload a CSV file containing genotypes, such as those generated by Kleborate.",
                                ),
                                ui.input_select(
                                    "genotype_flavour", "Genotype Format", choices={x.value: x.value.replace('-', ' ').replace('_', ' ').title() for x in GenotypeFlavour}
                                ),
                                ui.hr(),
                                ui.tooltip(
                                    ui.input_file(
                                ...
                                ui.input_select(
                                    "distance_flavour", "Distance Format", choices={x.value: x.value.replace('-', ' ').replace('_', ' ').title() for x in DistanceFlavour}
                                ),
                                ui.hr(),
                                ui.tooltip(
                                ...
                                ui.input_action_button(
                                    "btn_process", "Load Data", icon=icon_lucide("database"), class_="btn-primary w-100"
                                ),
                                class_="mt-3",
                            ),
                            icon=icon_lucide("folder"),
                        ),
                        ui.nav_panel(
                            "Pathogenwatch",
                            ui.div(
                                ...
                                ui.input_action_button(
                                    "btn_fetch_pw", "Fetch Collections", class_="btn-outline-primary w-100 mb-3"
                                ),
                                ui.input_selectize("pw_collection", "Select Collection", choices=[]),
                                ui.hr(),
                                ui.input_action_button(
                                    "btn_load_pw", "Load Data", icon=icon_lucide("database"), class_="btn-primary w-100"
                                ),
                                class_="mt-3",
                            ),
                            icon=icon_lucide("telescope"),
                        ),
                        ui.nav_panel(
                            "Example Data",
                            ui.div(
                                ui.p(
                                    "Load the KlebNET Neonatal Sepsis example dataset. This includes 981 "
                        ...
                        ui.nav_panel(
                            ...
                            ui.div(
                                ...
                                ui.input_action_button(
                                    "btn_fetch_kw", "Fetch Results", class_="btn-outline-primary w-100 mb-3"
                                ),
                                ui.input_selectize("kw_runs", "Select Run", choices=[]),
                                ui.hr(),
                                ui.input_action_button(
                                    "btn_load_kw", "Load Data", icon=icon_lucide("database"), class_="btn-primary w-100"
                                ),
                                class_="mt-3",
                            ),
                    ...
                    ui.hr(),
                    ui.input_action_button(
                        "btn_clear_data", "Clear Data", icon=icon_lucide("trash-2"), class_="btn-outline-danger w-100"
                    ),
                    icon=icon_lucide("database"),
                ),
                ui.accordion_panel(
                    ...
                    ui.input_action_button(
                        "btn_apply_qc", "Apply Filters", icon=icon_lucide("funnel"), class_="btn-primary w-100 mt-2"
                    ),
                    ui.input_action_button(
                        "btn_reset_qc",
                        "Remove Filters",
                        icon=icon_lucide("rotate-ccw"),
                        class_="btn-outline-secondary w-100 mt-2",
                    ),
                    icon=icon_lucide("check-check"),
                ),
                ui.accordion_panel(
                    "Cluster Generation",
                    ui.navset_pill(
                        ui.nav_panel(
                            ...
                            ui.div(
                                ui.p(
                                    "Identify genomic clusters using a SNP distance matrix.",
                                    class_="text-muted small mb-3",
                                ),
                                ui.tooltip(
                                ...
                                ui.input_action_button(
                                    "btn_calc_snp_clusters",
                                    "Calculate Clusters",
                                    icon=icon_lucide("network"),
                                    class_="btn-primary w-100 mt-2",
                                ),
                                class_="mt-3",
                            ),
                            icon=icon_lucide("dna"),
                        ),
                        ui.nav_panel(
                            "Transmission",
                            ui.div(
                                ui.p(
                                    "Identify outbreaks based on spatial and temporal proximity.",
                                    class_="text-muted small mb-3",
                                ),
                                ui.tooltip(
        ...
        ui.navset_card_tab(
            ui.nav_panel(
                "Dataset", ui.output_ui("dashboard_content"), value="tab_dataset_preview", icon=icon_lucide("search")
            ),
            ui.nav_panel(
                "Network",
                ui.layout_sidebar(
...
def dataset_server(input, output, session, app_state: dict):
    ...
    kw_runs_cache = app_state.setdefault("kw_runs_cache", reactive.Value({}))
    ...
    def dynamic_meta_mapping():
        if not (meta_info := input.metadata_file()):
            return ui.div()
        try:
            mapper = ColMapper(cols := pl.read_csv(Path(meta_info[0]["datapath"]), n_rows=1).columns)
            return ui.accordion(
                ui.accordion_panel(
                    ...
                    ui.layout_column_wrap(
                        ui.input_selectize(
                            "map_date", "Temporal", choices=[""] + cols, selected=mapper.guess("map_date")
                        ),
                        ui.input_select(
                            "map_date_res",
                            "Resolution",
                    ...
                    ui.layout_column_wrap(
                        ui.input_selectize(
                            "map_spatial", "Spatial", choices=[""] + cols, selected=mapper.guess("map_spatial")
                        ),
                        ui.input_select(
                            "map_spatial_res",
                            "Resolution",
                            choices={x.value: x.value.replace('-', ' ').replace('_', ' ').title() for x in SpatialResolution},
                            selected=SpatialResolution.COUNTRY.value,
                        ),
                        width=1 / 2,
                    ),
        ...
        async with ui_task("Pipeline Error") as p:
            try:
                ...
                if distance_info := input.distance_file():
                    ...
                    await sleep(0)
                    dist_obj = await to_thread(
                        GenomicDistances.from_file, Path(distance_info[0]["datapath"]), input.distance_flavour()
                    )
                ...
                ui.notification_show("Validation Error: Schema violation detected.", type="error", duration=15)
    ...
    @reactive.Effect
    @reactive.event(input.btn_fetch_kw)
    async def fetch_kw_runs():
        api_key = input.kw_api_key()
        if not api_key:
            ui.notification_show("Kaptive Web API Key is required", type="error")
            return

        async with ui_task("API Error") as p:
            p.set(message="Connecting to Kaptive Web...", value=30)
            await sleep(0)

            def fetch_runs():
                from seroepi.client import KaptiveClient

                with KaptiveClient(api_key=api_key) as client:
                    return client.get_runs()

            runs = await to_thread(fetch_runs)

            if not runs:
                ui.notification_show("No runs found.", type="warning")
                return

            # Note: Runs are dicts in the Kaptive API
            runs.sort(key=lambda r: r.get("name") or "", reverse=False)
            cache_map = {r["id"]: r for r in runs}
            choices_map = {"": "Select..."}
            choices_map.update({r["id"]: r.get("name") or f"Run {r['id'][:8]}" for r in runs})

            kw_runs_cache.set(cache_map)
            ui.update_selectize("kw_runs", choices=choices_map)
            p.set(message="Runs fetched!", value=100)
            ui.notification_show(f"Found {len(runs)} runs.", type="message")

    @reactive.Effect
    @reactive.event(input.btn_load_kw)
    async def load_kw_run():
        api_key, selected_id, cache = (
            input.kw_api_key(),
            input.kw_runs(),
            kw_runs_cache.get(),
        )
        if not api_key or not selected_id or selected_id not in cache:
            return
        run = cache[selected_id]

        async with ui_task("Load Error") as p:
            name = run.get("name") or f"Run {selected_id[:8]}"
            p.set(message=f"Fetching results for {name}...", value=30)
            await sleep(0)
            ...
            def fetch_results():
                ...
                with KaptiveClient(api_key=api_key) as client:
                    return client.get_run_results(selected_id, include_results=True)

            run_results = await to_thread(fetch_results)

            if not run_results or not run_results.get("results"):
                ui.notification_show("Run contains no results.", type="warning")
                return

            p.set(message="Parsing to DataFrame...", value=80)
            await sleep(0)

            from seroepi.io import KaptiveWebJSONParser

            df = await to_thread(KaptiveWebJSONParser.from_run_results, run_results, dataset_name=name)
            ...
            update_registry(dataset_registry, name, {"df": df, "dist": None, "trans_dist": None})
            app_state["active_dataset_name"].set(name)
            ui.notification_show(f"Loaded {len(df)} genomes from Kaptive Web.", type="message", duration=4)
    ...
    def qc_filters_ui():
        ...
        if "N50" in qc_cols:
            elements.append(
                ui.tooltip(
                    ui.input_numeric("qc_min_n50", "Minimum N50", value=10000), "Minimum required N50 assembly quality."
                )
            )
    ...
    async def apply_qc_filters():
        ...
        if not ds_name or ds_name not in reg:
            return
        ...
        async with ui_task("Filtering Error") as p:
            ...
            def run_filter():
                return base_df.qc.filter_assemblies(min_n50=min_n50, max_contigs=max_contigs, require_species=species)
    ...
    async def calc_snp_clusters():
        ...
        async with ui_task("SNP Clustering Error") as p:
            ...
            c_name = f"{Domain.CLUSTER.value}_snp_{input.snp_threshold()}"
            clusters = clusters.alias(c_name)
            p.set(message="Merging to dataset...", value=70)
            await sleep(0)
            df = df.with_columns(clusters)
            shared_df.set(df)
            ...
            if ds_name in dataset_registry.get():
                ...
                if unfilt_df is not None:
                    unfilt_df = unfilt_df.with_columns(clusters)
                updated_data = {**reg_data, "df": df, "unfiltered_df": unfilt_df}
                update_registry(dataset_registry, ds_name, updated_data)
            p.set(message="Done!", value=100)
            ui.notification_show(f"Successfully generated {c_name}", type="message")

    @reactive.Effect
    @reactive.event(input.btn_calc_trans_clusters)
    async def calc_trans_clusters():
        df = shared_df.get()
        if df is None or not input.trans_clone_col():
            return
        async with ui_task("Transmission Clustering Error") as p:
            ...
            c_name = getattr(t_clusters, "name", f"{Domain.CLUSTER.value}_transmission")
            t_clusters = t_clusters.alias(c_name)
            df = df.with_columns(t_clusters)
            shared_df.set(df)
    ...
    def dashboard_content():
        if (df := shared_df.get()) is None:  # noqa: F841
            return ui.div(
                "Please upload and process files to view the global dataset.", class_="text-center mt-5 text-muted fs-4"
            )
    ...
    def network_plot_content():
        ...
        if dist is None and trans_dist is None:
            return ui.div(
                "Calculate genomic or transmission clusters to view the network.",
    ...
    async def cluster_network_plot():
        ...
        if df is None:
            return Figure().update_layout(title="Upload a dataset to view the network.", **empty_theme)
        if dist is not None:
            primary_net = dist
            ...
            return Figure().update_layout(
                title="Upload a distance matrix or calculate transmission clusters.", **empty_theme
            )
