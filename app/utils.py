...
from seroepi.constants import BayesianInferenceMethod, Domain, PlotType
from seroepi.domains import render_plot

from .icons import icon_lucide as icon_lucide
...
def build_grouped_choices(columns: list[str], fallback_group: str = "Other", include_empty: bool = False) -> dict:
    domain_map = {
        ...
        Domain.VIRULENCE.value: "Virulence Markers",
        Domain.SPATIAL.value: "Spatial",
        Domain.TEMPORAL.value: "Temporal",
        Domain.CLUSTER.value: "Clusters",
    }
...
async def generate_temp_download(
    save_func: Callable[[Path], Any], suffix: str, error_prefix: str = "Export Error"
...
def export_settings_ui(prefix: str = "") -> ui.Tag:
    ...
    return ui.div(
        ...
        ui.tooltip(
            ui.input_select(f"{p}plot_format", "Format", choices=["png", "pdf", "svg", "jpeg"]),
            "The image format for the exported plot.",
        ),
        ui.tooltip(ui.input_numeric(f"{p}plot_width", "Width (px)", value=1200), "Exported image width in pixels."),
        ui.tooltip(ui.input_numeric(f"{p}plot_height", "Height (px)", value=800), "Exported image height in pixels."),
    )
    ...
    def build_ui(self, prefix: str = "est_param_", exclude: list[str] = None, default_overrides: dict = None) -> ui.Tag:
        ...
        for name, param in self.sig.parameters.items():
            ...
            if param.annotation is int or param.annotation is float:
                ...
                inp = ui.input_select(
                    input_id, label, choices={x.value: x.value.replace('-', ' ').replace('_', ' ').title() for x in BayesianInferenceMethod}, selected=default_val
                )
...
# Servers --------------------------------------------------------------------------------------------------------------
@module.server
def safe_plot_server(
    input, output, session, data_reactive: reactive.Value | Callable, plot_type: PlotType | str | Callable
):
...
def dt_download_server(input, output, session, data_callable: Callable, filename: str, height: str = "600px"):
    ...
    @render.download_button(filename=filename)
